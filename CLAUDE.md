...
- 보고 인용 시 원문 확인 필수 — 기억으로 재구성한 보고 내용(숫자·프레임 번호·질문의 존재)을 검증 없이 전제하지 않는다. 세션 트랜스크립트·`git log`·파일 내용이 근거다. 확인되지 않으면 "확인되지 않음"으로 보고하고, 그 인용에 기대어 문서·코드를 고치지 않는다. 사용자와 Claude 양쪽에 같이 적용된다.

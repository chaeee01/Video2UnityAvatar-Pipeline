...
| 항목 | 조건 | 근거 |
|---|---|---|
| 길이 | 90프레임(3초)-600프레임 | WHAM 추적 안정성 / 처리 시간 |
| fps | 24fps | 동작 재생 기준 (`docs/CONVENTIONS.md` 7절 표준, 소싱 4종 실측치) |
...
| 구간 | 첫 실행 | 이후 |
|---|---|---|
| 모델 다운로드 + 로드 | 약 80s | **약 56s** (다운로드만 생략, 볼륨→GPU 로드는 매번 발생) |
| 생성 | 약 6s | 약 6s |
| GLB 변환 (단순화 + 2048² 베이킹) | 약 20s | 약 20s |
| **총계** | **약 111s** | **약 85s** |

### 확인 포인트
- `params.json`의 `input_has_alpha`가 `true`인지 본다. `false`면 rembg가 배경을 뗀 것이라
  마스크 품질을 의심해야 한다 (로그에 `[경고] 알파 없음`이 찍힌다).
- `peak_vram_gb` **약 9.7GB** (24GB 중 40%). 크게 벗어나면 파라미터가 달라진 것이다.
